...
def import_uploaded_files(
    ...
    *,
    ...
    for item in files:
        imported.append(_import_one(upload_dir, item))
    return {"uploaded": imported, "tree": build_wiki_tree(workspace)}


def _import_one(upload_dir: Path, item: UploadedFile) -> dict[str, Any]:
    ...
    output_name = _unique_markdown_name(upload_dir, original_name)
...
def _unique_markdown_name(upload_dir: Path, original_name: str) -> str:
